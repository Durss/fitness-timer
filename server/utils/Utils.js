...
class Utils {
     ...
     * @returns {string}
     */
    static shorten(src, maxLength = 10) {
     ...
     * @returns {string[]}
     */
    static smartSplitString(str, maxLength = 140) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            if (length + w.length + 1 > maxLength) {
                let line = str.substr(offset, length).trim();
                if (line.length > 0) {
                    strings.push(line);
                }
                offset += length;
        ...
        if (offset < str.length) {
            let line = str.substr(offset, str.length - offset).trim();
            if (line.length > 0)
                strings.push(line);
        }
        return strings;
    ...
    static replaceDiacritics(str) {
        if (!this.diacriticsMap)
            this.initDiacritics();
        return str.replace(/[^\u0000-\u007E]/g, (a) => {
            return this.diacriticsMap[a] || a;
        });
    }
    static initDiacritics() {
        this.diacriticsMap = {};
     ...
     */
    static getMostSimilarIpFrom(ref) {
...
Utils.defaultDiacriticsRemovalMap = [
    {
        'base': 'A',
